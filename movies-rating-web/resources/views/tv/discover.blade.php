...
<head>
    ...
    <title>Discover Series | CinePals</title>
    ...
    <section class="container mt-3">
        ...
        <div class="row flex-row g-4 mt-3 align-items-start">
            ...
            <div class="border col-3 p-3 position-sticky" style="top: 80px;">
                ...
                <form action="{{ route('tv.discover') }}" method="GET" id="genreForm">
            ...
            <div class="col-9 px-3">
                <div id="trendingSection"
                    ...
                    @if(empty($discoverTv['results']))
                        ...
                        @endfor
                    @else
                        @foreach($discoverTv['results'] as $tv)
                            <a href="{{ route('tv.detail', $tv['id']) }}" class="text-decoration-none">
                                <div class="card border-0" style="width:170px;">
                                    <img src="https://image.tmdb.org/t/p/w500{{ $tv['poster_path'] }}"
                                         alt="{{ $tv['name'] }}"
                                         class="rounded"
                                         style="width:170px; height:255px; object-fit:cover;">
                                    <p class="mt-2 text-center small fw-semibold">{{ $tv['name'] }}</p>
                                </div>
                            </a>
                        @endforeach
                    @endif
                ...
                @if(isset($discoverTv['page']))
                    <div class="d-flex justify-content-center mt-4">
                        ...
                        @endphp
                        @if($discoverTv['page'] > 1)
                            <a href="{{ route('tv.discover', ['genre' => $currentGenres, 'page' => $discoverTv['page'] - 1]) }}"
                               class="btn btn-outline-secondary me-2">Previous</a>
                        @endif
                        @if($discoverTv['page'] < $discoverTv['total_pages'])
                            <a href="{{ route('tv.discover', ['genre' => $currentGenres, 'page' => $discoverTv['page'] + 1]) }}"
